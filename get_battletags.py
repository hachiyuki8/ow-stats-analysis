...
NAME_PROGRESS = "usednames.txt" # file name to store all usernames for which searching is done
...
def getBattletagsFromName(username):
    ...
    if response.status_code == 200:
        ...
        print(f"[getBattletagsFromName] Error querying {username} from playoverwatch.com\n")
...
def getTagsFromSet(names, output):
    """ For every given username, find all the associated battletags and write them to the given 
        ...
        username is added to a file, defined by NAME_PROGRESS, which contains all usernames for 
        which the searching is done.
    ...
    name_count = 0
    tag_count = 0
    with open(NAME_PROGRESS, "a+") as usedNames:
        for name in names:
...
if __name__ == "__main__":
    """ Write all valid battletags (PC, public, level >= 25) associated with usernames stored in 
        ...
        usernames in NAME_PROGRESS from text files in NAMES_PATH before rerunning the program. 
        NAME_PROGRESS and OUTPUT_FILE can remain the same across multiple runs.
    """
